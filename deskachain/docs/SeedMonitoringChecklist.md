...
```powershell
.\deskachain.exe --rpc-url http://127.0.0.1:9311 peer check http://<seed-host>:10311/
```
...
```powershell
.\deskachain.exe --rpc-url http://127.0.0.1:9311 chain validate
```

Systemd logs:

```sh
sudo systemctl status deskachain-testnet --no-pager
