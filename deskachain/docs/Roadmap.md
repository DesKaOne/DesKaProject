...
## Phase 2.6.6 — Protocol Spec Freeze & iND Base58 Address Migration
...
* Replace dev address format with final iND Base58 address.
...
IndoChain now uses secp256k1 for new wallet keys, compressed public keys, and transaction signatures.
...
* Add iND Base58Check address encoder.
* Add iND Base58Check address decoder.
...
* Final `iND...` Base58Check address encoder/decoder with checksum and leading-zero safe Base58.
...
* Add service node identity tied to `iND...` owner address.
...
* Keep service points outside dIDR balances and consensus.
...
* Keep service agent behavior outside consensus and dIDR balances.

Tasks:

* Add `indoservice` binary.
...
* Add dIDR lock/unlock staking collateral without changing PoW consensus.
...
* Keep staking collateral-only with no PoS, no validators, no APY, no dIDR staking rewards, and no slashing.
...
* Load seeds from flags, config, `IND_SEED_PEERS`, and seed files.
...
* Add version metadata to `indochain`, `indominer`, and `indoservice`.
...
* Keep testnet dIDR valueless and mainnet unavailable.
...
* Pool server.
* Stratum-like protocol or simple dIDR pool protocol.
* Worker registration.
* Share difficulty.
* Share validation.
* Payout accounting.
...
* Add `examples/systemd/indochain-testnet.env`.
...
* Keep testnet dIDR no-monetary-value language explicit.
...
* Harden `indominer` retry/backoff, stale job detection, submit timeout behavior, and stats output.
...
## Phase 6 — Swap / Wrapped dIDR Research

Status: Planned after stable testnet

Goals:

* Let users eventually swap dIDR value without rushing exchange listing.
