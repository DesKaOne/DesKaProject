...
Testnet dIDR has no monetary value. Staking remains collateral-only. Service points are simulation-only and are not spendable dIDR.
...
- search by height, block hash, transaction id, or iND address.
...
- `iND...` address for the active network.
...
`/explorer/status` returns network identity, tip, difficulty, maturity, supply, pending transaction count, peer count, RPC mode flags, `mainnet_available: false`, and a warning that testnet dIDR has no monetary value.
...
Service points are simulation-only and are not dIDR.
