...
DesKaChain adalah blockchain kecil eksperimental berbasis CPU mining yang ditulis dengan Go.

Fase saat ini: Phase 4.12 - Peer Discovery & Auto Bootstrap.

Peringatan: DesKaChain adalah software blockchain lokal yang masih eksperimental. Jangan gunakan wallet Phase 1.5 untuk dana sungguhan. Private key wallet disimpan secara lokal hanya untuk kenyamanan pengembangan.
...
DesKaChain tidak lagi bergantung pada fixed difficulty. Phase 2.7 menambahkan aturan retarget konservatif untuk eksperimen localnet/testnet.
...
```powershell
go run ./node/cmd/deskachain --network testnet --datadir ./data/testnet node start --public-rpc --rpc :18331 --p2p :19331 --advertise-p2p http://127.0.0.1:19331 --cors-origins https://explorer.example
```
...
```powershell
go run ./node/cmd/deskachain node start --config ./docs/config.example.json
```
...
Saat `node start` berjalan, jangan jalankan command tulis lokal terhadap datadir yang sama dari process lain. Gunakan `--rpc-url` untuk mengontrol node yang sedang berjalan. DesKaChain menulis `<datadir>/node.lock` saat node berjalan untuk mengurangi risiko accidental concurrent writer.
...
Height yang sama tidak berarti chain yang sama. Node hanya dianggap up to date ketika height dan tip hash sama. Jika dua node memiliki height yang sama tapi tip hash berbeda, DesKaChain menganggapnya sebagai fork. Peer sync menolak forked chain sampai Phase 2.5 memperkenalkan safe automatic reorg support.
