...
func TestV3SnapshotRoundTripKeepsAssets(t *testing.T) {
	...
	if err := l.ApplyBlock(types.Block{Height: 1, MinerAddress: w.Address, Transactions: []types.Transaction{coinbase}}); err != nil {
		t.Fatal(err)
	}
	create := types.NewAssetCreateTransaction(w.Address, "Example", "EXT", 6, 1000, true, true, false, false, 1, 1)
	if err := w.SignTransactionWithProfile(&create, profile); err != nil {
		t.Fatal(err)
	}
	if err := l.ApplyBlock(types.Block{Height: 2, MinerAddress: w.Address, Transactions: []types.Transaction{create}}); err != nil {
		t.Fatal(err)
	}

	mint := types.NewAssetMintTransaction(w.Address, create.AssetID, w.Address, 10, 1, 2)
	if err := w.SignTransactionWithProfile(&mint, profile); err != nil {
		t.Fatal(err)
	}
	if err := l.ApplyBlock(types.Block{Height: 3, MinerAddress: w.Address, Transactions: []types.Transaction{mint}}); err != nil {
		t.Fatal(err)
	}
...
func TestV3SnapshotRejectsNativeAccountBalanceMismatch(t *testing.T) {
	...
	if err := snapshot.Validate(); err == nil {
		t.Fatal("snapshot accepted mismatched native dIDR/account balance")
	}
}
