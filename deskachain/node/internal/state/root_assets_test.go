...
func TestStateRootIncludesIssuedAssetAndBalances(t *testing.T) {
	...
	assets := []asset.Definition{
		{ID:asset.NativeAssetID, Name:"IDR", Symbol:"IDR", Decimals:0, Kind:asset.KindFungible, Issuer:"protocol", Status:asset.StatusActive},
	...
	if err != nil { t.Fatal(err) }
	if rootA == rootB { t.Fatal("asset balance mutation did not change state root") }
}
