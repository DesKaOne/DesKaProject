...
func TestDerivedAssetID(t *testing.T) {
	if got := DerivedID("ABCDEF"); got != "asset:abcdef" { t.Fatalf("derived asset id = %q", got) }
	if IsNative("asset:abcdef") { t.Fatal("issued asset id treated as native") }
}
...
func TestValidateDefinition(t *testing.T) {
	...
	if err := ValidateDefinition(valid); err != nil { t.Fatal(err) }
	bad := valid; bad.ID = NativeAssetID
	if err := ValidateDefinition(bad); err == nil { t.Fatal("native asset accepted as issued asset") }
}