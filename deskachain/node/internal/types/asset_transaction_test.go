...
func TestAssetTransactionUsesNativeINDFeeModel(t *testing.T) {
	tx := NewAssetTransferTransaction("from", "to", "asset:usd", 1500, 7, 1)
	if tx.ProtocolVersion() != TxVersionAsset {
		t.Fatalf("version=%d", tx.ProtocolVersion())
	}
	if tx.EffectiveAssetID() != "asset:usd" {
		t.Fatalf("asset=%q", tx.EffectiveAssetID())
	}
	if tx.EffectiveFeePayer() != "from" {
		t.Fatalf("fee payer=%q", tx.EffectiveFeePayer())
	}
	if tx.Fee != 7 {
		t.Fatalf("fee=%d", tx.Fee)
	}
}
...
func TestAssetTransactionCanonicalRoundTrip(t *testing.T) {
	...
	decoded, err := DecodeCanonicalTransaction(raw)
	if err != nil {
		t.Fatal(err)
	}
	if decoded != tx {
...
func TestAssetSigningBindsFeePayerAndAsset(t *testing.T) {
	...
	a, err := tx.SigningBytesWithChainID(777001)
	if err != nil {
		t.Fatal(err)
	}
	tx.FeePayer = "other"
	b, err := tx.SigningBytesWithChainID(777001)
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Equal(a, b) {
		t.Fatal("fee payer not bound into v3 signing domain")
	}
}
...
func TestFeePayerAuthorizationDoesNotDependOnSponsorSignature(t *testing.T) {
	...
	a, err := tx.FeePayerSigningBytesWithChainID(777001)
	if err != nil {
		t.Fatal(err)
	}
	tx.FeePayerPublicKey = "pub2"
	tx.FeePayerSignature = "sig"
	b, err := tx.FeePayerSigningBytesWithChainID(777001)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(a, b) {
		t.Fatal("paymaster signature fields changed authorization bytes")
	}
}
...
func TestValidateAssetEnvelope(t *testing.T) {
	...
	badMint := NewAssetMintTransaction("issuer", "dIDR", "alice", 10, 1, 1)
	if err := badMint.ValidateAssetEnvelope(); err == nil {
		t.Fatal("native dIDR mint accepted as issued asset operation")
	}
}
