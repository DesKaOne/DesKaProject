...
type explorerIndexCursor struct {
	Height uint64
	Hash   string
}
...
type explorerIndexedHistory struct {
	TxID         string
	BlockHeight  uint64
	BlockHash    string
	Role         string
...
type explorerIndexedAssetEvent struct {
	TxID        string
	BlockHeight uint64
	BlockHash   string
	AssetID     string
...
func newExplorerIndexer(paths config.Paths, profile config.NetworkConfig) *explorerIndexer {
	return &explorerIndexer{paths: paths, profile: profile}
}
func (x *explorerIndexer) dbPath() string {
	return filepath.Join(x.paths.DataDir, explorerIndexerFileName)
}
func (x *explorerIndexer) open() (*bolt.DB, error) {
	if err := os.MkdirAll(x.paths.DataDir, 0755); err != nil {
		return nil, err
	}
	...
	err = db.Update(func(tx *bolt.Tx) error {
		for _, n := range []string{"meta", "blocks", "txs", "addresses", "assets"} {
...
func clearExplorerIndexTx(tx *bolt.Tx) error {
	for _, n := range []string{"blocks", "txs", "addresses", "assets"} {
		b := tx.Bucket([]byte(n))
		if b == nil {
			continue
		}
	...
	return tx.Bucket([]byte("meta")).Delete([]byte("cursor"))
}
func uint64Key(v uint64) []byte { b := make([]byte, 8); binary.BigEndian.PutUint64(b, v); return b }
func historyKey(address string, h uint64, txid, role string) []byte {
	return []byte(fmt.Sprintf("%s|%020d|%s|%s", address, h, txid, role))
}
func assetEventKey(id string, h uint64, txid string) []byte {
	return []byte(fmt.Sprintf("%s|%020d|%s", id, h, txid))
}
func findBlockByHeight(blocks []types.Block, h uint64) *types.Block {
	for i := range blocks {
...
func historyAddress(item types.Transaction, role string) string {
	switch role {
	case "from":
		return item.From
	case "to":
...
func indexExplorerBlockTx(tx *bolt.Tx, block types.Block) error {
	br, err := json.Marshal(explorerIndexedBlock{Height: block.Height, Hash: block.Hash, PreviousHash: block.PreviousHash, Timestamp: block.Timestamp, TxCount: len(block.Transactions)})
	if err != nil {
		return err
	}
	if err = tx.Bucket([]byte("blocks")).Put(uint64Key(block.Height), br); err != nil {
		return err
	}
	for i, item := range block.Transactions {
		ix := explorerIndexedTx{ID: item.ID, BlockHeight: block.Height, BlockHash: block.Hash, Index: i, From: item.From, To: item.To, FeePayer: item.EffectiveFeePayer(), AssetID: item.EffectiveAssetID(), Amount: item.Amount, Fee: item.Fee, Status: "confirmed"}
		raw, err := json.Marshal(ix)
		if err != nil {
			return err
		}
		if err := tx.Bucket([]byte("txs")).Put([]byte(item.ID), raw); err != nil {
			return err
		}
		events := []explorerIndexedHistory{}
		if item.From != "" {
			events = append(events, explorerIndexedHistory{TxID: item.ID, BlockHeight: block.Height, BlockHash: block.Hash, Role: "from", Counterparty: item.To, AssetID: item.EffectiveAssetID(), Amount: item.Amount, Fee: item.Fee})
		}
		if item.To != "" && item.To != item.From {
			events = append(events, explorerIndexedHistory{TxID: item.ID, BlockHeight: block.Height, BlockHash: block.Hash, Role: "to", Counterparty: item.From, AssetID: item.EffectiveAssetID(), Amount: item.Amount, Fee: item.Fee})
		}
		if ix.FeePayer != "" && ix.FeePayer != item.From && ix.FeePayer != item.To {
			events = append(events, explorerIndexedHistory{TxID: item.ID, BlockHeight: block.Height, BlockHash: block.Hash, Role: "fee_payer", Counterparty: item.To, AssetID: item.EffectiveAssetID(), Amount: item.Amount, Fee: item.Fee})
		}
		for _, e := range events {
			...
			if er = tx.Bucket([]byte("addresses")).Put(historyKey(historyAddress(item, e.Role), e.BlockHeight, e.TxID, e.Role), r); er != nil {
				return er
			}
		}
		if ix.AssetID != "" && !asset.IsNative(ix.AssetID) {
			e := explorerIndexedAssetEvent{TxID: item.ID, BlockHeight: block.Height, BlockHash: block.Hash, AssetID: ix.AssetID, From: item.From, To: item.To, Amount: item.Amount, Fee: item.Fee}
			r, er := json.Marshal(e)
			if er != nil {
				return er
			}
			if er = tx.Bucket([]byte("assets")).Put(assetEventKey(e.AssetID, e.BlockHeight, e.TxID), r); er != nil {
				return er
			}
		}
	}
...
func (x *explorerIndexer) sync() (ExplorerIndexerStatus, error) {
	...
	if len(blocks) == 0 {
		return ExplorerIndexerStatus{}, errors.New("chain is empty")
	}
	db, err := x.open()
	if err != nil {
		return ExplorerIndexerStatus{}, err
	...
	defer db.Close()
	var cur explorerIndexCursor
	_ = db.View(func(tx *bolt.Tx) error {
		r := tx.Bucket([]byte("meta")).Get([]byte("cursor"))
		if r != nil {
			return json.Unmarshal(r, &cur)
		}
		return nil
	})
	rebuild := false
	if cur.Hash != "" {
		if b := findBlockByHeight(blocks, cur.Height); b != nil && b.Hash != cur.Hash {
			rebuild = true
		}
	}
	if cur.Height > blocks[len(blocks)-1].Height {
		rebuild = true
	}
	if rebuild {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		...
		for _, b := range blocks {
			...
			if b.Height > 0 {
				p := findBlockByHeight(blocks, b.Height-1)
				if p == nil || p.Hash != b.PreviousHash {
					return fmt.Errorf("canonical parent mismatch at height %d", b.Height)
				}
			}
...
func (x *explorerIndexer) statusUnlocked() (ExplorerIndexerStatus, error) {
	...
	lag := uint64(0)
	if tip.Height > c.Height {
		lag = tip.Height - c.Height
	}
...
func (x *explorerIndexer) transaction(id string) (explorerIndexedTx, bool, error) {
	db, err := x.open()
	if err != nil {
		return explorerIndexedTx{}, false, err
	}
	defer db.Close()
	var out explorerIndexedTx
	err = db.View(func(tx *bolt.Tx) error {
		r := tx.Bucket([]byte("txs")).Get([]byte(id))
		if r == nil {
			return nil
		}
		return json.Unmarshal(r, &out)
	})
	return out, out.ID != "" && err == nil, err
}
func (x *explorerIndexer) blockByHeight(height uint64) (explorerIndexedBlock, bool, error) {
	db, err := x.open()
	if err != nil {
		return explorerIndexedBlock{}, false, err
	}
	defer db.Close()
