(function () {
  ...
  function renderMining() {
    ...
    jsonFetch("/mining/blocks?limit=10").then(function (data) {
      ...
      app.innerHTML = panel("Mining / Difficulty", '<div class="grid">' + metrics + "</div><p class=\"muted\">Difficulty observation is informational only. Testnet DKC has no monetary value.</p>") +
  ...
  function renderAddress(address) {
    ...
    Promise.all([
      ...
      app.innerHTML = panel("Address detail", '<div class="grid">' + metrics + "</div><p class=\"muted\">Service points are simulation-only and are not spendable DKC.</p>") +
  ...
  function renderServices() {
    ...
    jsonFetch("/explorer/services?limit=" + serviceLimit + "&offset=" + serviceOffset).then(function (data) {
      ...
      app.innerHTML = panel("Service nodes", '<p class="muted">Service points are simulation-only and are not spendable DKC. Service state is local to this RPC node.</p>' +
  ...
  function runSearch(q) {
    q = String(q || "").trim();
    if (!q) {
      setError("Enter a block height, block hash, transaction id, or DKC address.");
      return;
    }
    input.value = q;
    setLoading("Search");
