...
func TestFaucetToStakeToServiceEligibleE2E(t *testing.T) {
	...
	if !strings.Contains(score["note"].(string), "not spendable DKC") {
...
func TestChainInfoCirculatingSupplyWithMaturity(t *testing.T) {
	...
	if info["total_supply"] != "6100 DKC" {
		t.Fatalf("total supply = %v want 6100 DKC", info["total_supply"])
	}
	if info["circulating_supply"] != "1100 DKC" {
		t.Fatalf("circulating supply = %v want mature coinbase supply 1100 DKC", info["circulating_supply"])
	}
}
...
func TestChainInfoCirculatingSupplyIncludesActiveStake(t *testing.T) {
	...
	if info["circulating_supply"] != "1350 DKC" {
		t.Fatalf("circulating supply = %v want 1350 DKC with active stake still counted", info["circulating_supply"])
	}
	if info["total_active_stake"] != "1000 DKC" {
		t.Fatalf("total active stake = %v want 1000 DKC", info["total_active_stake"])
	}
}
...
func TestChainInfoSpendableNotEqualCirculating(t *testing.T) {
	...
	if chainInfo["circulating_supply"] != "1350 DKC" || balance["spendable_balance"] != "0" || balance["active_stake"] != "1000" {
...
func TestChainInfoSupplyUnaffectedByFaucetStakeService(t *testing.T) {
	...
	info := getRPCMap(t, fixture.serverURL+"/chain/info", http.StatusOK)
	if info["total_supply"] != "6350 DKC" || afterFaucetMine-before != config.InitialBlockReward || afterStakeMine-afterFaucetMine != config.InitialBlockReward || totalSupplyFixture(t, fixture.paths) != afterStakeMine {
...
func newFaucetStakeServiceFixture(t *testing.T, faucetAmountDKC uint64) faucetStakeServiceFixture {
	t.Helper()
	info := faucetInfo()
	info.FaucetAmount = faucetAmountDKC * config.UnitsPerCoin
